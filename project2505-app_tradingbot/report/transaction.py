...
def main():
    ...
    # We'll compute:
    #   total_qty_bought[symbol]   = sum of Qty where Action == "Buy"
    #   total_qty_sold[symbol]     = sum of Qty where Action == "Sell"
    #   cash_spent[symbol]         = sum of Amount_float where Action == "Buy"  (these are negative)
    #   cash_received[symbol]      = sum of Amount_float where Action == "Sell" (these are positive)
    #
    # Net position = total_qty_bought - total_qty_sold
    # Net cash     = cash_received + cash_spent
    ...
    # # Calculate annual dividend income only for stocks with recent dividend data
    # combined["Annual_Dividend_Income"] = combined.apply(
    #     lambda row: round(row["Net_Qty"] * row["Latest_Dividend"] * 4, 2) 
    #     if pd.notna(row["Last_Dividend_Date"]) else 0.0,
    #     axis=1
    # )
    ...
    # # Print summary of dividend-paying stocks (only those with recent dividend data)
    # dividend_stocks = combined[
    #     (combined["Latest_Dividend"] > 0) & 
    #     (combined["Last_Dividend_Date"].notna())
    # ]
    # if not dividend_stocks.empty:
    #     print("\nDividend-paying stocks in your portfolio (with recent dividend data):")
    #     for symbol in dividend_stocks.index:
    #         print(f"{symbol}: ${dividend_stocks.loc[symbol, 'Latest_Dividend']:.2f} per share ({dividend_stocks.loc[symbol, 'Div_Yield_Pct']:.2f}% yield)")
    #         print(f"  Last paid: {dividend_stocks.loc[symbol, 'Last_Dividend_Date']}")
    #         print(f"  Next expected: {dividend_stocks.loc[symbol, 'Anticipated_Ex_Date']}")
    #     print(f"\nEstimated annual dividend income: ${combined['Annual_Dividend_Income'].sum():.2f}")
    # else:
    #     print("\nNo recent dividend-paying stocks found in your portfolio.")
