...
const_obj_image_tutorial_2 = pygame.image.load(r'pictures\presentation_tuto_2.png').convert()
...
def main(character_player):
    # Background Music
    play_background_music(const_music_background_1)
    ...
    # Introduction
    loading_background()
    if not character_player.seen_intro_tutorial:
        ...
        conversation('Illene: ',
                     'You passed out and temporarily forget everything',
                     'because I just made you awaken your ability.',
                     "I'm Illene.")

        conversation('Illene: ',
                     "I'm half vampire.",
                     "And this is because of DIO who is the first vampire",
                     "appeared in the world.")

        conversation('Illene: ',
                     "This place is a virtual system space of Passione",
                     "for training.",
                     "We, passione, must kill DIO so all the vampires",
                     "disappear.")

        play_background_music(const_music_main_theme)
        presentation(const_obj_image_tutorial_2)
        ...
        conversation("Sexy pistols: ",
                     "Mista, you smell!!!!!!!! shxx!!")

        conversation('Mista: ',
                     'What is this??',
                     'What are these small weirdos?')

        conversation('Illene: ',
                     "That's the ability I said",
                     "Defeat DIO with them.")

        conversation('Illene: ',
                     "Well... experience what that is.",
                     "Turn on the simulator!!")

        character_player.seen_intro_tutorial = True

    conversation('Mission Object: ',
                 "1. Defeat all the Illusions of Illene")
    ...
    while not var_gameExit:
        ...
        if is_list_death(enemy_list) and boss_object_1.is_death() and is_list_death(illene_list) and is_list_death(killer_list):
            conversation('Ellene: ',
                         'Great, Mista',
                         "I think you will be alright without me.",
                         'I will go first, follow me.')
            conversation('+EXP 3000')

            character_player.game_win()
            character_player.increase_exp(3000)
            var_gamewin = True
        while var_gamewin:
            good_message('Mission Complete', 1)
            good_message('', 2)
            good_message('congulatulations!', 3)
            good_message('You just finished the tutorial!', 4)
            pygame.display.update()
        ...
        if not conv:
            ...
            conversation('Illene: ',
                         "Press space if you wanna play the game faster.")
