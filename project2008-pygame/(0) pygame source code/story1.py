...
const_background_size_x = 4
const_background_size_y = 4
const_random_list = [] 
...
def main(character_player):
    # Background Music
    play_background_music(const_music_story_1)
    ...
    # Introduction
    loading_background()
    if not character_player.seen_intro_1:
        obj_screen.fill((0,0,0))
        conversation("Passione: ",
                     '3!$%@!%#!%.')

        conversation("Passione: ",
                     'Mista, Illene is now in really dangerous situation',
                     'There were too many vampires more than we ',
                     'expected, support her and defeat the rest of the ',
                     'vampires.')

        conversation("Passione: ",
                     "Don't touch vampire's body.",
                     'They all are infected.')

        presentation(const_obj_image_presentation_1)

        conversation("Mista: ",
                     'Crap.. Vampires are everywhere..')

        conversation("Mista: ",
                     'Crap.. Vampires are everywhere..',
                     'Wait for a second Illene,',
                     'I will make you go back to Passione!')

        conversation("Mista: ",
                     'Crap.. Vampires are everywhere..',
                     'Wait for a second Illene,',
                     'I will make you go back to Passione!',
                     'I think I have to pay attention.')
        ...
        conversation("Mista: ",
                     ...
                     'I will make you go back to Passione!',
                     'I think I have to pay attention.',
                     "Why they all have the same hoody as Illene's..?!")
    
        character_player.seen_intro_1 = True

    conversation('Mission Object: ',
                    "1. Defeat all the vampires",
                    '2. Save Illene')
    ...
    while not var_gameExit:
        ...
        if character_player.is_death() or is_list_death(illene_list):
            conversation('Illene: ' 
                         'Mi..Mista...')
            conversation('-EXP 20%')

            character_player.game_over()
            var_gameover = True
        while var_gameover:
            alert_message('Game Over', 1)
            alert_message('What to do next?', 2)
            alert_message('Press C to try the mission again', 3)
            alert_message('Press Q to quit the mission', 4)
        ...
        if is_list_death(enemy_list):
            conversation('Illene: ' 
                         'Mi..Mista... Thank you')
            conversation('Mista: ' 
                         'Are you alright?',
                         "I will beat DIO instead of you")
            conversation('Mista: ' 
                         'Are you alright?',
                         "I will beat DIO instead of you",
                         'Go back to Passione')
            ...
            var_gamewin = True
        while var_gamewin:
            good_message('Mission Complete', 1)
            good_message('', 2)
            good_message('congulatulations!', 3)
            good_message('You just finished the mission #1', 4)
            pygame.display.update()
        ...
        show_background(back_x, back_y, const_obj_image_1_1, const_obj_image_1_2, const_obj_image_1_3)
        if boss_exist and boss_object_1.is_THE_WORLD():
            show_the_world_background(back_x, back_y, const_obj_image_the_world)
        ...
        if is_click_button(740, 530):
            ...
            conversation('You lost some portion of your EXP.', '-EXP 20%')
