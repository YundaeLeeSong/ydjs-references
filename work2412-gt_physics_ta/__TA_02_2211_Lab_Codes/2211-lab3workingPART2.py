...
while t < tmax:
    ...
    rmag_bh_to_planet = mag(r_bh_to_planet)
    rhat_bh_to_planet = norm(r_bh_to_planet)
    Fmag_bh_ON_planet = (G * bh.m * planet.m) / (rmag_bh_to_planet**2)
    F_bh_ON_planet = Fmag_bh_ON_planet * (-rhat_bh_to_planet)
    
    r_ship_to_planet = planet.pos - ship.pos
    rmag_ship_to_planet = mag(r_ship_to_planet)
    rhat_ship_to_planet = norm(r_ship_to_planet)
    Fmag_ship_ON_planet = (G * planet.m * ship.m) / (rmag_ship_to_planet**2)
    F_ship_ON_planet = -Fmag_ship_ON_planet * rhat_ship_to_planet
    ...
    rmag_bh_to_ship = mag(r_bh_to_ship)
    rhat_bh_to_ship = norm(r_bh_to_ship)
    Fmag_bh_ON_ship = (G * bh.m * ship.m) / (rmag_bh_to_ship**2)
    F_bh_ON_ship = -Fmag_bh_ON_ship * rhat_bh_to_ship
    
    r_planet_to_ship = ship.pos - planet.pos
    rmag_planet_to_ship = mag(r_planet_to_ship)
    rhat_planet_to_ship = norm(r_planet_to_ship)
    Fmag_planet_ON_ship = (G * planet.m * ship.m) / (rmag_planet_to_ship**2)
    F_planet_ON_ship = -Fmag_planet_ON_ship * rhat_planet_to_ship
