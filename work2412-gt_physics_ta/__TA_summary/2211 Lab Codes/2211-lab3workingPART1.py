...
while idx <(len(X)-1):  #iterate over data values
    ...
    rmag = mag(r)
    rsquared = rmag**2
    mBH = ( mag(Fnet) * rsquared ) / (G*star.m)
    print(mBH)
    ...
    arrowFnet.axis = Fnet*arrowscale            # orange
    arrowFnet_par.pos = star.pos
    arrowFnet_par.axis = Fnet_par*arrowscale    # magenta
    arrowFnet_perp.pos = star.pos
    arrowFnet_perp.axis = Fnet_perp*arrowscale  # cyan
...
# Printing the forces -- EDIT THE NEXT THREE LINES
print("|Fnet|=",mag(Fnet))
